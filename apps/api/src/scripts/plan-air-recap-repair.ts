...
async function run(): Promise<Record<string, unknown>> {
  if (!process.env.TVDB_API_KEY?.trim()) {
    fail("TVDB_API_KEY is required for the AIR recap repair planner");
  }
  ...
  assertExactRange(
    authoritative.map((episode) => episode.providerEpisodeNumber),
    1,
    RECAP_NUMBER,
    "AIR TVDB season 1",
  );
  if (new Set(authoritative.map((episode) => episode.providerEpisodeId)).size !== authoritative.length) {
    fail("AIR TVDB season contains duplicate provider episode IDs");
  }
  ...
  if (
    ...
    fail(
      `TVDB episode 13 no longer matches the proven AIR recap evidence: title=${recap.title} airDate=${recap.airDate} season=${recap.seasonNumber}`,
    );
  }
  ...
  if (providerEntities.length !== 1) {
    fail(`Expected exactly one provider entity for ${PROVIDER}:${PROVIDER_ID}`);
  }
  ...
  if (
    ...
    fail(
      `Legacy AIR TVDB owner is not the expected safely-retirable Airs mapping: anime=${legacyOwner.animeId} source=${legacyOwner.source} confidence=${legacyOwner.confidence}`,
    );
  }

  const targetLegacy = await queryRows<LegacyMappingRow>(sql`
    select id, anime_id as "animeId", source, confidence, is_primary as "isPrimary"
    from public.anime_mappings
    where anime_id = ${TARGET_ANIME_ID} and provider = ${PROVIDER}
  `);
  if (targetLegacy.length !== 0) {
    fail("AIR already has a legacy TVDB mapping; refusing to plan around mixed state");
  }
  ...
  const v2Associations = await queryRows<V2AssociationRow>(sql`
    ...
    order by apm.id
  `);
  if (v2Associations.length !== 1) {
    fail(`Expected exactly one current v2 association for ${PROVIDER}:${PROVIDER_ID}`);
  }
  ...
  if (
    ...
    fail(
      `V2 AIR TVDB owner is not the expected safely-retirable Airs association: anime=${v2Owner.animeId} segments=${v2Owner.segmentCount} source=${v2Owner.source} confidence=${v2Owner.confidence}`,
    );
  }
  ...
  `);
  if (targetV2.length !== 0) {
    fail("AIR already has a v2 TVDB association; refusing to plan around mixed state");
  }
  ...
  assertExactRange(
    normalEpisodes.map((episode) => episode.number),
    1,
    12,
    "AIR normal local episodes",
  );
  if (localEpisodes.some((episode) => episode.number === RECAP_NUMBER)) {
    fail("AIR already has a local episode numbered 13; refusing duplicate materialization");
  }
  ...
  if (byProviderId.size !== episodeMappings.length) {
    fail("Duplicate TVDB provider episode IDs exist in AIR season scope");
  }
  ...
  for (const authoritativeEpisode of authoritative) {
    ...
    if (number === 1) {
      ...
      if (
        ...
        Number(current.providerEpisodeNumber) !== 1
      ) {
    ...
    if (number <= 12) {
      ...
      if (
        !current ||
        current.animeId !== TARGET_ANIME_ID ||
        current.episodeId !== target.id ||
        current.localEpisodeNumber !== number ||
        current.localKind !== "normal" ||
        Number(current.providerEpisodeNumber) !== number
      ) {
    ...
    if (current) {
      fail("AIR TVDB recap episode 13 is already mapped; expected it to be unmapped");
    }
  }
  ...
  return {
    ...
    operation: {
      ...
      description:
        "Plan the verified AIR TVDB correction without writing: retire the bogus Airs parent, promote AIR as the whole-season parent, move TVDB episode 1, and materialize TVDB episode 13 as a recap rather than a normal episode.",
      provider: PROVIDER,
        ...
        createEpisodes: [
          {
            animeId: TARGET_ANIME_ID,
            number: RECAP_NUMBER,
            sortNumber: RECAP_NUMBER,
            kind: "recap",
            title: recap.title,
            titleEnglish: recap.title,
            synopsis: recap.overview,
            airDate: recap.airDate,
            seasonNumber: recap.seasonNumber,
          },
        ],
        createEpisodeMappings: [
          {
            provider: PROVIDER,
            providerEpisodeId: recap.providerEpisodeId,
            providerEpisodeNumber: String(RECAP_NUMBER),
            source: "system",
            confidence: 95,
          },
        ],
...
if (import.meta.main) {
  try {
    ...
    if (args.length > 0) {
      fail(`This planner is dry-run only and accepts no arguments; received: ${args.join(" ")}`);
    }
